...
export default function SignalsPage() {
  ...
  const signals = [
    ...
    { id: 2, symbol: 'XAU/USD', type: 'SELL', entry: 2015.50, sl: 2025.00, tp1: 2005.00, tp2: 1995.00, status: 'pending', channel: 'Gold Signals', sentAt: null, recipients: 0 },
    { id: 3, symbol: 'GBP/JPY', type: 'BUY', entry: 185.50, sl: 184.80, tp1: 186.50, tp2: 187.00, status: 'draft', channel: 'Forex Signals', sentAt: null, recipients: 0 },
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                <div className={'w-2 h-2 rounded-full ' + (
                  signal.status === 'sent' ? 'bg-green-500' : 
                  signal.status === 'pending' ? 'bg-yellow-500' : 'bg-gray-400'
                )} />
              ...
              <span className={'px-2 py-1 text-xs font-medium rounded-full ' + (
                signal.status === 'sent' ? 'bg-green-100 text-green-800' : 
                signal.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'
              )}>
                {signal.status}
              </span>
            </div>
